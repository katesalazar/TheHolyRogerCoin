...
# Copyright (c) 2018-2020 TheHolyRoger Core developers
...
def build():
    ...
    subprocess.check_call(['make', '-C', '../TheHolyRogerCoin/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-signed', '--destination', '../gitian.sigs.theholyroger/', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win-signer.yml'])
        try:
    ...
    if args.macos:
        ...
        subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs.theholyroger/', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        try:
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.theholyroger/', '-r', args.version+'-linux', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.theholyroger/', '-r', args.version+'-win-unsigned', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.theholyroger/', '-r', args.version+'-osx-unsigned', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.theholyroger/', '-r', args.version+'-win-signed', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win-signer.yml']):
        print('Verifying v'+args.version+' Signed Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.theholyroger/', '-r', args.version+'-osx-signed', '../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/TheHolyRogerCoin/TheHolyRogerCoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.chdir('TheHolyRogerCoin')
    if args.pull:
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
        os.chdir('../gitian-builder/inputs/TheHolyRogerCoin')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
