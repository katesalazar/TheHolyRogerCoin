...
const char * const BITCOIN_CONF_FILENAME = "theholyroger.conf";
const char * const BITCOIN_PID_FILENAME = "theholyrogerd.pid";
...
#else
    const char* pszModule = "theholyroger";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TheHolyRoger";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TheHolyRoger";
#else
    // Unix
    return pathRet / ".theholyroger";
#endif
#endif
}
